...
func main() {
    ...
    for _,seed_num := range splitSpaceSepNums(seed_list_str) {
        seed_list = append(seed_list, SeedLocation{ seed_num, -1, -1, -1 , -1, -1, -1, -1 })
    }
    ...
    for _,current_seed := range seed_list {
        current_seed.soil_num = useLut(mapping_seed_soil, current_seed.seed_num)
