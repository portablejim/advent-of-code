...
func main() {
    ...
    for _, current_part := range part_list {
        adjacent_part_num_indicies := []int{};
        ...
        */
        ...
        for _, num_index := range num_index_list {
            ...
            if num_index >= 0 {
                // Build a list of unique indicies adjacent to the gear.
                add_index := true
                for _,n := range adjacent_part_num_indicies {
                    if n == num_index {
                        add_index = false
                    }
                }
                if add_index {
                    adjacent_part_num_indicies = append(adjacent_part_num_indicies, num_index)
                }
            }
        }

        // Do the calculation for the target gears.
        if len(adjacent_part_num_indicies) == 2 {
            fmt.Printf("Gear ratio: ")
            gear_ratio := 1
            for _,nIndex := range adjacent_part_num_indicies {
                target_number := numbers_list[nIndex]
                fmt.Printf("* %d ", target_number)
                gear_ratio = gear_ratio * int(target_number)
            }
            total += uint64(gear_ratio)
            fmt.Printf("| %d | %d\n", gear_ratio, total)
        }
    }
